...
void calculate(float i, float j){
    float x0 = 0.5f * (2 * i * i - j * j) * (i * i + j * j - 1) + i * (i * i - 3 * j * j);
    float y0 = 0.5f * (2 * j * j - i * i) * (i * i + j * j - 1) + j * (j * j - 3 * i * i);
    float z0 = (i * i + j * j) * (i * i + j * j - 1);
...
int main(){
    ...
    while(1){
        ...
        for(U = -1.5; U <= 1.5; U += 0.03){
            for(V = -1.5; V <= 1.5; V += 0.03){
                calculate(U, V);
            }
        }
        printf("\x1b[H");
